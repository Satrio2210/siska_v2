  ...
  if (aksesku.readyState==4)
    ...
    if(data.length>1)
      ...
      {
        document.getElementById('txtmanucode').setAttribute('disabled','true');
        document.getElementById('txtmanuname').setAttribute('disabled','true');
    ...
    if(data.length>1)
      {
        ...
        swal({
            title: 'Code is Exist' ,
            text: 'Kode Manual sudah ada',
            icon: 'warning',
        });
      }
      else
      {
        document.getElementById("txtmanuname").removeAttribute('disabled');
...
// end periksa Kode
  ...
  // Input Data Posisi dari form ke Tabel tbllmanu

  var ajaxinput;
  function input(manucode,manuname)
  {
    ajaxinput = buatajaxinput();
    var url="TBLLMANU01E.php";
    ajaxinput.onreadystatechange=stateChangedInput;
    var params = "q="+manucode+"|"+manuname;
    //alert(params);
...
var ajaxview
function viewcode(manucode)
{
  try 
  {
    ajaxview = buatajaxview();
    var url="TBLLMANU01C.php";
    ajaxview.onreadystatechange=stateChangedView;
    var params = "q="+manucode;
    //alert(params);
  ...
  if (ajaxview.readyState==4)
    {
      ...
      {
      var res = data.split("|");
      document.getElementById("txtmanucode").value = res[1];
      document.getElementById("txtmanuname").removeAttribute('disabled');
      document.getElementById("txtmanuname").value = res[2];
      }      
        //}
    }
  }
...
// Hapus 
  var ajaxhapus;
  function hapuscode(manucode)
  {
    ajaxhapus = buatajaxhapus();
    var url="TBLLMANU01D.php";
