  ...
  if (aksesku.readyState==4)
    ...
    if(data.length>1)
      ...
      {
        document.getElementById('txtvarncode').setAttribute('disabled','true');
        document.getElementById('txtvarnname').setAttribute('disabled','true');
        document.getElementById('txtvarnnote').setAttribute('disabled','true');
  ...
  if (ajaxku.readyState==4)
    ...
    if(data.length>1)
      ...
      else
      {
        ...
        document.getElementById("txtvarnnote").removeAttribute('disabled');
...
// end periksa Kode
  ...
  // Input Data Posisi dari form ke Tabel tblivarn

  var ajaxinput;
  function input(varncode,varnname,varnnote)
  {
    ajaxinput = buatajaxinput();
    //var url="TBLIUNIT01E.php";
    var url="TBLIVARN01E.php";
    ajaxinput.onreadystatechange=stateChangedInput;
    var params = "q="+varncode+"|"+varnname+"|"+varnnote;
    //alert(params);
  ...
  if (ajaxinput.readyState==4)
    {
        ...
        document.getElementById('txtvarnname').value = '';
        document.getElementById('txtvarnname').setAttribute('disabled','true');
        document.getElementById('txtvarnnote').value = '1';
        document.getElementById('txtvarnnote').setAttribute('disabled','true');
        ambilscreen('');
        document.getElementById('txtvarncode').focus();
        //}
  ...
  if (ajaxview.readyState==4)
    {
      ...
      {
      var res = data.split("|");
      document.getElementById("txtvarncode").value = res[1];
      document.getElementById("txtvarnname").removeAttribute('disabled');
      document.getElementById("txtvarnname").value = res[2];
      document.getElementById("txtvarnnote").removeAttribute('disabled');
      document.getElementById("txtvarnnote").value = res[3];
      }      
        //}
    }
  }
