...
function stateChangedView() {
  ...
  if (ajaxview.readyState == 4) {
    ...
    if (data.length > 1) {
      ...
      var saleMode = (res[15] || 'BAYAR').trim();
      if (document.getElementById("hidsalemode")) {
        document.getElementById("hidsalemode").value = saleMode;
      }
      applySaleModeUI(saleMode, res[5], parseFloat(res[6]) || 0);

      hitungKembalian();
      ambilviewinvc(res[1]);
      console.log('stateChangedView: form populated', { saleMode });
...
/**
 * BPJS:
 *  - CLOSE  : tidak ada layanan/tindakan berbayar (>0)
 *  - BAYAR  : ada layanan/tindakan berbayar (hanya itu yang ditagih)
 */
function applySaleModeUI(saleMode, regipaym, paymtota) {
  var btn = document.getElementById("btnSaleAction");
  var paym = document.getElementById("txtpaymamnt");
  var mode = document.getElementById("optpaymmode");
  var disc = document.getElementById("txtpaymdisc");

  if (!btn) return;

  if (regipaym === 'B' && saleMode === 'CLOSE') {
    btn.textContent = 'Close!';
    btn.className = 'btn-modern btn-delete';
    btn.style.height = '38px';
    if (paym) {
      paym.value = '0';
      paym.setAttribute('disabled', 'true');
    }
    if (mode) mode.setAttribute('disabled', 'true');
    if (disc) disc.setAttribute('disabled', 'true');
    document.getElementById("txtpaymkemb").value = '0';
  } else {
    btn.textContent = 'Bayar!';
    btn.className = 'btn-modern btn-save';
    btn.style.height = '38px';
    if (paym) paym.removeAttribute('disabled');
    if (mode) mode.removeAttribute('disabled');
    if (disc) disc.removeAttribute('disabled');
    if (paym) paym.focus();
  }
}

function handleSaleAction() {
  var saleMode = document.getElementById('hidsalemode')
    ? document.getElementById('hidsalemode').value
    : 'BAYAR';
  ...
  // alur bayar normal (termasuk BPJS dengan tambahan berbayar)
  if (document.getElementById('txtpaymtota').value == '') {
    swal({ title: 'Total yang harus di bayar Kosong', text: 'Anda belum memilih Pasien yang akan melakukan pembayaran', icon: 'warning' });
    return;
  }
  if (document.getElementById('txtpaymtota').value <= 0) {
    swal({ title: 'Telah di lakukan Pembayaran', text: 'Pasien telah melakukan pembayaran, konfirmasikan ke Admisi untuk di tutup', icon: 'warning' });
    return;
  }
  if (isNaN(document.getElementById('txtpaymamnt').value) && isNaN(convertToAngka(document.getElementById('txtpaymamnt').value))) {
    swal({ title: 'Salah Input Angka', text: 'Masukkan Nominal Angka , silahkan periksa lagi', icon: 'warning' });
    return;
  }
  var amntRaw = convertToAngka(document.getElementById('txtpaymamnt').value);
  var totaRaw = document.getElementById('hidpaymtota').value;
  if (parseFloat(amntRaw) <= 0) {
    swal({ title: 'Pembayaran Nol ', text: 'Isi kembali Nominal pembayaran, silahkan periksa lagi', icon: 'warning' });
    return;
  }
  if (parseFloat(amntRaw) < parseFloat(totaRaw)) {
    swal({ title: 'Pembayaran Kurang ', text: 'Isi kembali Nominal pembayaran, silahkan periksa lagi', icon: 'warning' });
    return;
  }
  if (document.getElementById('txtpaymamnt').value == '') {
    swal({ title: 'Nominal Pembayaran Kosong', text: 'Anda belum mengisi Nominal Pembayaran, silah periksa lagi', icon: 'warning' });
    return;
  }
...
function closeBpjsPasien() {
  var regicode = document.getElementById('hidregicode').value;
  var paticode = document.getElementById('hidpaticode').value;
  if (!regicode) {
    swal({ title: 'Data kosong', text: 'Pilih pasien terlebih dahulu', icon: 'warning' });
    return;
  }

  swal({
    title: 'Tutup pasien BPJS?',
    text: 'Tidak ada tagihan tambahan. Status akan menjadi Sudah Bayar.',
    icon: 'warning',
    buttons: true
  }).then(function (ok) {
    if (!ok) return;
    $.ajax({
      url: 'TRXASALE01E-CLOSE.php',
      type: 'POST',
      dataType: 'json',
      data: { q: regicode + '|' + paticode },
      success: function (res) {
        if (res && res.status === 'ok') {
          swal({ title: 'Sukses', text: res.message || 'Pasien ditutup', icon: 'success' })
            .then(function () {
              window.location.href = 'TRXASALE00.php';
            });
        } else {
          swal({ title: 'Gagal', text: (res && res.message) || 'Gagal close', icon: 'error' });
        }
      },
      error: function (xhr) {
        swal({ title: 'Error', text: xhr.responseText || 'Gagal request', icon: 'error' });
      }
    });
  });
}
