    ...
    if(data.length>1)
      {
        ...
        document.getElementById('tglinvcdate').setAttribute('disabled','true');
...
function isipocode(outproccode,outdueddate,outsuplname,outsuplcode)
{
	try 
	{
  document.getElementById('txtinvccode').removeAttribute('disabled','true');
  document.getElementById('tglinvcdate').removeAttribute('disabled','true');

  document.getElementById('txtproccode').removeAttribute('disabled','true');
  document.getElementById('txtproccode').value = outproccode;

  document.getElementById('tgldueddate').removeAttribute('disabled','true');
  ...
  document.getElementById("txtinvccode").focus();
...
// Create Invoice
  var ajaxinput;
  function input(invccode,invcdate,proccode,dueddate,suplcode)
  {
    ajaxinput = buatajaxinput();
    var url="TRXAPROC05E.php";
    ajaxinput.onreadystatechange=stateChangedInput;
    var params = "q="+invccode+"|"+invcdate+"|"+proccode+"|"+dueddate+"|"+suplcode;   
    //alert(params);
