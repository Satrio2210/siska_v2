    ...
    if(data.length>1)
      {
        ...
        document.getElementById('optpatient').setAttribute('disabled','true');
        document.getElementById('optdate').setAttribute('disabled','true');
...
// end periksa akses  
  ...
  if (mediku.readyState==4)
    ...
    if(data.length>1)
      {
        document.getElementById('txtattcode').value=data;
      }
      else
      {
        document.getElementById('txtattcode').value='';
      }
    }
  }
...
function isipolicode(outpolicode,outpoliname)
{
  ...
  document.getElementById('optpatient').removeAttribute('disabled');
  document.getElementById('optdate').removeAttribute('disabled');
...
//input(inattcode,inmastroom,inmastpaym,inmastuser,inmastrate,innomiamnt);
  var ajaxinput;
  function input(attcode,mastroom,mastpaym,mastuser,mastrate,nomiamnt)
  {
    ajaxinput = buatajaxinput();
    var url="TRXAMEDI03E.php";
    ajaxinput.onreadystatechange=stateChangedInput;
    var params = "q="+attcode+"|"+mastroom+"|"+mastpaym+"|"+mastuser+"|"+mastrate+"|"+nomiamnt;
    //alert(params);
...
var ajaxview
function viewcode(attcode)
{
  try 
  {
    ajaxview = buatajaxview();
    var url="TRXAMEDI03C.php";
  ...
  if (ajaxview.readyState==4)
    {
      ...
      var res = data.split("|");
      document.getElementById("txtattcode").value = res[1];
      ...
      if (res[7] == 'P') 
        {
        document.getElementById("optpatient").checked = true;
        document.getElementById("optdate").checked = false;
        document.getElementById("hidmastrate").value = res[7];
        }

      else if (res[7] == 'D') 
        {
        document.getElementById("optpatient").checked = false;
        document.getElementById("optdate").checked = true;
        document.getElementById("hidmastrate").value = res[7];
        }
...
// Hapus 
  var ajaxhapus;
  function hapuscode(attcode)
  {
    ajaxhapus = buatajaxhapus();
    var url="TRXAMEDI03D.php";
